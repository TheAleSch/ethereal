        ...
        <div className="flex gap-1 rounded border border-white/10 bg-white/5 p-1">
          {THEME_ROW.map((t) => (
            <button
              ...
              className={cn(
                "hit-44-tight flex min-w-0 flex-1 items-center justify-center gap-1 truncate rounded px-1 py-1 text-[11px] font-medium capitalize transition-colors sm:px-1.5",
                (
                  isBaseConfig
                    ? baseTarget === (t.key === "light" ? "base" : "dark")
                    : (previewTheme ?? "light") === t.key
