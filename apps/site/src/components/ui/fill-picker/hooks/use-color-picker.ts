...
export function useColorPicker(
  ...
  const shouldRestoreHue =
    isControlledStringInput && (controlledParsed?.hueMissing ?? true)
  // Read the ref *after* the write above so this render sees the hue it just
  // remembered; holding it in a plain local keeps it usable as a memo dep.
  const rememberedHue = lastGoodHueRef.current
  const color = React.useMemo<OklchColor>(
    () => (shouldRestoreHue ? { ...rawColor, h: rememberedHue } : rawColor),
    [shouldRestoreHue, rawColor, rememberedHue]
  )
