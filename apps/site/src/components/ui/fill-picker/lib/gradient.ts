...
/**
 * True when a top-level segment parses as a color stop (`<color>` or
 * `<color> <position>%`). The radial/conic branches use this to detect the
 * omitted-prelude form CSS allows (`radial-gradient(red, blue)`), where the
 * first segment is already the first stop and must not be consumed as a
 * shape/position/angle prelude.
 */
function isColorStopSegment(segment: string): boolean {
  const positionMatch = segment.match(/^(.*?)\s+(-?\d+(?:\.\d+)?)%$/);
  const colorStr = positionMatch ? positionMatch[1].trim() : segment;
...
export function parseGradient(input: string): Gradient | null {
  ...
  if (type === "radial") {
    ...
    // Hand-written CSS may omit the prelude entirely (`radial-gradient(red,
    // blue)`) — mirror the linear branch and keep a first segment that is
    // really a color stop instead of consuming it.
    const hasPrelude = !isColorStopSegment(parts[0]);
    const { interp, rest } = hasPrelude
      ? extractInterp(parts[0])
      : { interp: "oklch" as GradientInterp, rest: "" };
    const head = rest;
    const stopParts = hasPrelude ? parts.slice(1) : parts;
  ...
  // As with radial, the prelude may be omitted (`conic-gradient(red, blue)`)
  // — keep a first segment that is really a color stop.
  const hasPrelude = !isColorStopSegment(parts[0]);
  const { interp, rest } = hasPrelude
    ? extractInterp(parts[0])
    : { interp: "oklch" as GradientInterp, rest: "" };
